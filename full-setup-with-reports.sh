...
# Script completo: Reset + Seed + Popular Reports
...
echo -e "${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  🚀 SETUP COMPLETO - DO ZERO ATÉ REPORTS FUNCIONANDO          ║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
docker-compose exec app php artisan reports:seed-all-domains --sync $LIMIT $DATE_FROM $DATE_TO

echo ""
echo -e "${GREEN}✅ Reports criados!${NC}"
echo ""

echo -e "${GREEN}✅ Reports criados e processados sincronamente!${NC}"
echo ""
...
echo 'TOKEN=$(curl -s http://localhost:8007/api/admin/login \'
echo '  -H "Content-Type: application/json" \'
echo '  -d '"'"'{"email":"admin@dashboard.com","password":"password123"}'"'"' | jq -r '"'"'.token'"'"')'
echo ""

echo -e "${YELLOW}📊 2. Dashboard do zip.50g.io:${NC}"
echo 'curl -s http://localhost:8007/api/admin/reports/domain/1/dashboard \'
echo '  -H "Authorization: Bearer $TOKEN" | jq ".data.kpis"'
echo ""

echo -e "${YELLOW}🏆 3. Ranking Global:${NC}"
echo 'curl -s http://localhost:8007/api/admin/reports/global/domain-ranking \'
